...
def get_backtest_history(dollar_inv, rebalData, roundup = False, tradeCost = 0.01):
    ...
    for i in tqdm(range(len(rebal_date))):
        ...
        if i < len(rebal_date)-1:
            ...
            inv_money_list += [dollar_ongoing]
            inv_money_date += list(basket_price.index[1:])  
            tradeCost_history[''.join([x for x in str(lastDay)[:10] if x != '-'])] = tradingCost
            turnover_history[''.join([x for x in str(lastDay)[:10] if x != '-'])] = tradingCost / dollar_ongoing
            weight_history[''.join([x for x in str(lastDay)[:10] if x != '-'])] = weightData    
