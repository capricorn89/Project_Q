...
trainloader = torch.utils.data.DataLoader(mnist_train, shuffle=True)
testloader = torch.utils.data.DataLoader(mnist_test, shuffle=False)


#%% Visualizing random MNIST traindata
def Visual_mnist(train):
    data = (train.train_data).numpy()
    label = (train.train_labels).numpy()
    ran = np.random.randint(len(data))
    plt.title('Label is {label}'.format(label=label[ran]))
    plt.imshow(data[ran])
...
def Visual_mnist_test(mode):  # network를 다 학습시킨 다음, testset에서 visualization. Feedforward는 'F', Convolutional은 'C', GoogleNet은 'G'
    ran = np.random.randint(len(mnist_test))
...
#%% Basic Feedforward Net (mnist) ----- 82.5%, 2번 돌리면 72%
class Net(nn.Module):
    def __init__(self):
...
for epoch, data in enumerate(trainloader):
    inputs, labels = data
    inputs = inputs.view(1,28*28)  # net에 넣기 전에 데이터 모양 맞춰주기
...
torch.save(goog.state_dict(), "C:/Users/User/Desktop/업무/data/feed.pt")  # net부분에 내가 저장하길 원하는 모델 넣기
